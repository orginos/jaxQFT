...
def action_grad_links_autodiff(theory: U1YangMills, U: Array) -> Array:
...
def force_from_action_autodiff(theory: U1YangMills, U: Array) -> Array:
    """Build Lie force from autodiff scalar gradient:
    X = U * (dS/dU), F = proj_u1(X).
    """
...
def force_action_autodiff_test(
    ...
    for _ in range(max(1, int(n_trials))):
        ...
        dS_dU = action_grad_links_autodiff(theory, U)
        F_ad = project_u1_algebra(U * dS_dU)
...
def main():
    ...
    ap.add_argument(
        ...
        type=str,
        default="all",
        help="comma-separated subset of: layout,timing,fd,autodiff,eps2,eps4,gauge,forcecov,forceimpl,selfcheck,all",
    )
    ...
    if args.selfcheck:
        ...
        if "all" in tests:
            tests = {"layout", "timing", "fd", "autodiff", "eps2", "eps4", "gauge", "forcecov", "forceimpl"}
