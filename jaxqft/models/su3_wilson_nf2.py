...
class SU3WilsonNf2(SU3YangMills):
    ...
    def _pack_even_sites(self, psi: Array) -> Array:
        if psi.ndim == self.Nd + 2:
            sites = psi.reshape((self._n_sites, self.Ns, 3))
            return jnp.take(sites, self._cb_even_idx, axis=0)
        if psi.ndim == self.Nd + 3:
            sites = psi.reshape((psi.shape[0], self._n_sites, self.Ns, 3))
    ...
    def _unpack_even_sites(self, psi_even: Array) -> Array:
        if psi_even.ndim == 3:
            sites = jnp.zeros((self._n_sites, self.Ns, 3), dtype=psi_even.dtype)
            sites = sites.at[self._cb_even_idx].set(psi_even, indices_are_sorted=True, unique_indices=True)
            return sites.reshape((*self.lattice_shape, self.Ns, 3))
        if psi_even.ndim == 4:
            sites = jnp.zeros((psi_even.shape[0], self._n_sites, self.Ns, 3), dtype=psi_even.dtype)
            sites = sites.at[:, self._cb_even_idx].set(psi_even, indices_are_sorted=True, unique_indices=True)
            return sites.reshape((psi_even.shape[0], *self.lattice_shape, self.Ns, 3))
        raise ValueError(f"Unexpected compact even rank: {psi_even.ndim}")

    def _unpack_odd_sites(self, psi_odd: Array) -> Array:
        if psi_odd.ndim == 3:
            sites = jnp.zeros((self._n_sites, self.Ns, 3), dtype=psi_odd.dtype)
            sites = sites.at[self._cb_odd_idx].set(psi_odd, indices_are_sorted=True, unique_indices=True)
            return sites.reshape((*self.lattice_shape, self.Ns, 3))
        if psi_odd.ndim == 4:
            sites = jnp.zeros((psi_odd.shape[0], self._n_sites, self.Ns, 3), dtype=psi_odd.dtype)
            sites = sites.at[:, self._cb_odd_idx].set(psi_odd, indices_are_sorted=True, unique_indices=True)
            return sites.reshape((psi_odd.shape[0], *self.lattice_shape, self.Ns, 3))
    ...
    def pseudofermion_force_from_solution(self, U: Array, X: Array, Y: Array) -> Array:
        ...
        for mu in range(self.Nd):
            ...
            links_force.append(project_su3_algebra(k))
...
def test_eo_operator(th: SU3WilsonNf2, n_iter: int = 10) -> Dict[str, float]:
    U = th.hot_start(scale=0.05)
    psi = th.random_fermion()
...
def _effective_refresh_eta(th: SU3WilsonNf2, U: Array) -> Optional[Array]:
    """Return eta associated with current pseudofermion state for refresh diagnostics.

    In OU mode we may only carry phi explicitly. Recover eta from phi via the
    current normal solve so refresh tests remain meaningful.
    """
...
def main():
    ...
    ap.add_argument(
        ...
        type=str,
        default="all",
        help="comma-separated: gamma,adjoint,normal,perf,eo,eoperf,eps2,eps4,reversibility,reproducibility,hamiltonian,pfrefresh,pfid,pfsolve,pfcov,gauge,conventions,forcecmp,eopforce,mcmcsmoke,all",
    )
    args = ap.parse_args()
    ...
    if "mcmcsmoke" in tests:
        ...
        s = run_nf2_mcmc_smoke_suite("SU3WilsonNf2", _factory, config=cfg)
