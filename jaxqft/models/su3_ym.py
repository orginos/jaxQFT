"""Pure SU(3) Yang-Mills theory in link coordinates for HMC."""
...
class SU3YangMills:
    ...
    def __post_init__(self):
        ...
        self.layout = layout_alias.get(self.layout, self.layout)
        ...
        if self.exp_method not in ("expm", "su3"):
    ...
    def field_shape(self):
        if self.layout == "BMXYIJ":
            return (self.Bs, self.Nd, *self.lattice_shape, 3, 3)
        return (self.Bs, *self.lattice_shape, self.Nd, 3, 3)
    ...
    def _roll(self, x: Array, shift: int, direction: int) -> Array:
        # Site-shaped tensors [B, *L, 3, 3] do not carry the mu axis.
        if x.ndim == self.Nd + 3:
            ax = 1 + direction
        # Link-shaped tensors keep the dedicated mu axis.
    ...
    def algebra_to_links(self, q: Array) -> Array:
        # q is anti-hermitian traceless algebra field, per-link.
        q = project_su3_algebra(q)
        if self.exp_method == "su3":
            from jaxqft.LieGroups import su3 as su3_lg
    ...
    def _staple(self, U: Array, mu: int) -> Array:
        ...
        for nu in range(self.Nd):
            if nu == mu:
                continue
            U_nu = self._take_mu(U, nu)
            # Standard staple V_mu(x), transforming as a link:
            # V -> Omega^\dagger(x) V Omega(x+mu)
            # forward:  U_nu(x) U_mu(x+nu) U_nu^\dagger(x+mu)
            fwd = U_nu @ self._roll(U_mu, -1, nu) @ _dagger(self._roll(U_nu, -1, mu))
            # backward: U_nu^\dagger(x-nu) U_mu(x-nu) U_nu(x-nu+mu)
    ...
    def force_reference(self, U: Array) -> Array:
        ...
        for mu in range(self.Nd):
            ...
            G_mu = coef * project_su3_algebra(U_mu @ _dagger(staple_mu))
            links_force.append(G_mu)
        if self.layout == "BMXYIJ":
            return jnp.stack(links_force, axis=1)
...
def benchmark_exponentiation(
    ...
    for _ in range(max(1, n_iter)):
        th_su3.algebra_to_links(q).block_until_ready()
    t3 = time.perf_counter()

    rel = float(jnp.linalg.norm(Us - Ue) / (jnp.linalg.norm(Ue) + 1e-12))
    eye = jnp.eye(3, dtype=Ue.dtype)
    ue_u = _dagger(Ue) @ Ue
    us_u = _dagger(Us) @ Us
    unit_expm = float(jnp.mean(jnp.linalg.norm(ue_u - eye, axis=(-2, -1))))
    unit_su3 = float(jnp.mean(jnp.linalg.norm(us_u - eye, axis=(-2, -1))))
    det_expm = float(jnp.mean(jnp.abs(jnp.linalg.det(Ue) - 1.0)))
    det_su3 = float(jnp.mean(jnp.abs(jnp.linalg.det(Us) - 1.0)))

    return {
        "expm_sec_per_call": (t1 - t0) / max(1, n_iter),
        "su3_sec_per_call": (t3 - t2) / max(1, n_iter),
        "rel_diff_su3_vs_expm": rel,
        "unitarity_expm": unit_expm,
        "unitarity_su3": unit_su3,
        "det_err_expm": det_expm,
        "det_err_su3": det_su3,
    }


def force_action_fd_test(theory: SU3YangMills, eps: float = 1e-5) -> Dict[str, float]:
    U = theory.hot_start(scale=0.05)
    H = theory.refresh_p()

    # Manifold-consistent directional derivative:
    # U(eps) = exp(eps H) U with H in su(3).
    ...
    # Derivative inferred from force contraction for the left variation
    # U(eps) = exp(eps H) U, where force = -grad.
    F = theory.force(U)
...
def action_grad_links_autodiff(theory: SU3YangMills, U: Array) -> Array:
    """Return entrywise dS/dU using real-imag autodiff on S=sum_batch action(U)."""
    ...
    if grad_links_fn is None:
        ...
        def _grad_links(u: Array) -> Array:
            ...
            # For real-valued S(U), entrywise directional derivative is:
            # dS = Re sum_ij (dS/dU_ij) dU_ij  with dS/dU = g_re - i g_im.
...
def force_from_action_autodiff(theory: SU3YangMills, U: Array) -> Array:
    """Build Lie force from autodiff matrix gradient:
    dS/dU_ij = G_ji, X = U G, F = proj_su3(X).
    """
...
def force_action_autodiff_test(
    ...
    for _ in range(max(1, int(n_trials))):
        ...
        dS_dU = action_grad_links_autodiff(theory, U)
        F_ad = project_su3_algebra(U @ jnp.swapaxes(dS_dU, -1, -2))
        ...
        # Cross-check on a random Lie direction H using the same manifold variation:
        # U(eps) = exp(eps H) U, so dU = H U at eps=0.
        H = theory.refresh_p()
        dU = H @ U
...
def force_action_autodiff_timing(
    ...
    # Build and cache autodiff gradient callable outside timing region.
...
def epsilon2_test(
    ...
    from jaxqft.core.integrators import leapfrog, minnorm2, minnorm4pf4, force_gradient
...
def kernel_timing(theory: SU3YangMills, n_iter: int = 10) -> Dict[str, float]:
    ...
    t3 = time.perf_counter()
    return {
        "action_sec_per_call": (t1 - t0) / n_iter,
        "force_sec_per_call": (t3 - t2) / n_iter,
    }
...
def random_site_gauge_with_method(theory: SU3YangMills, scale: float = 0.2, method: str = "expm") -> Array:
    method = method.lower()
    if method not in ("expm", "su3"):
        raise ValueError("gauge exp method must be expm or su3")
    site_shape = (theory.Bs, *theory.lattice_shape, 3, 3)
    ...
    a = project_su3_algebra(a)
    if method == "su3":
        from jaxqft.LieGroups import su3 as su3_lg

        return su3_lg.expo(a)
    return jax.scipy.linalg.expm(a)


def gauge_transform_links(theory: SU3YangMills, U: Array, Omega: Array) -> Array:
    links_mu = []
    for mu in range(theory.Nd):
        U_mu = theory._take_mu(U, mu)  # [B,*L,3,3]
...
def full_selfcheck(
    ...
    alg = su3_lg.check_algebra()
    alg_norm = float(alg["norm_check"])
    alg_comm = float(alg["comm_check"])
    alg_d = float(alg["d_check"])
    alg_ok = (alg_norm < 1e-5) and (alg_comm < 1e-5) and (alg_d < 1e-5)
    checks.append(("lie_algebra", alg_ok, f"norm={alg_norm:.3e}, comm={alg_comm:.3e}, d={alg_d:.3e}"))
    ...
    exb_rel = float(exb["rel_diff_su3_vs_expm"])
    exb_ok = exb_rel <= exp_rel_tol
    checks.append(("expo", exb_ok, f"rel_diff={exb_rel:.3e}, tol={exp_rel_tol:.1e}"))
    ...
    g_ok = g_rel < 1e-5
    checks.append(("gauge_invariance", g_ok, f"max_rel_action_diff={g_rel:.3e}"))
    ...
    fd_ok = fd_rel < 5e-3
    checks.append(("fd_force_consistency", fd_ok, f"rel_fd_vs_force={fd_rel:.3e}"))
    ...
    ad_ok = (ad_rel < 5e-3) and (ad_dir < 5e-3)
    checks.append(
        (
    ...
    eps2 = epsilon2_test(
        ...
        tau=1.0,
        nmd_values=(8, 16, 32),
        integrator_name="minnorm2",
    )
    slope = float(eps2["loglog_slope"])
    eps2_ok = (slope > 1.5) and (slope < 2.5)
    checks.append(("eps2_scaling", eps2_ok, f"slope={slope:.3f}"))
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="SU3 Yang-Mills diagnostics (force/action/epsilon2/layout timing)",
        allow_abbrev=False,
    )
    ...
    ap.add_argument("--exp-method", type=str, default="auto", choices=["expm", "su3", "auto"])
    ap.add_argument("--exp-rel-tol", type=float, default=1e-5, help="max relative diff for auto choosing su3")
    ...
    if "layout" in tests or args.layout == "auto":
        ...
        if args.layout == "auto":
            # prioritize force timing for HMC cost
    ...
    if "expo" in tests or args.exp_method == "auto":
        ...
        print(f"  su3  sec/call: {exb['su3_sec_per_call']:.6e}")
        print(f"  rel_diff(su3,expm): {exb['rel_diff_su3_vs_expm']:.6e}")
        print(f"  unitarity expm/su3: {exb['unitarity_expm']:.6e} / {exb['unitarity_su3']:.6e}")
        print(f"  det err  expm/su3: {exb['det_err_expm']:.6e} / {exb['det_err_su3']:.6e}")
        if args.exp_method == "auto":
            if exb["rel_diff_su3_vs_expm"] <= args.exp_rel_tol and exb["su3_sec_per_call"] < exb["expm_sec_per_call"]:
                selected_exp_method = "su3"
            else:
    ...
    if "fd" in tests:
        ...
        print(
            "  mean fd / -<F,H>:"
            f" {fd['mean_fd']:.6e} / {fd['mean_force_minus']:.6e}"
        )
