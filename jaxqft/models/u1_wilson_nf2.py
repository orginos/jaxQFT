"""U(1) gauge + Nf=2 Wilson-fermion model with monomial Hamiltonian composition."""
...
def _algebra_inner(a: Array, b: Array) -> Array:
    """Positive-definite inner product on anti-Hermitian U(1) algebra: -Re(a b)."""
    trab = jnp.real(a * b)
...
class GaugeActionMonomial:
    ...
    def action(self, q: Array) -> Array:
        return U1YangMills.action(self.model, q)

    def force(self, q: Array) -> Array:
        return U1YangMills.force(self.model, q)
...
class WilsonNf2PseudofermionMonomial:
    ...
    def _build_force_ad_fn(self):
        ...
        def force_fn(u: Array, phi: Array) -> Array:
            ...
            return project_u1_algebra(u * dS_dU)
...
class U1WilsonNf2(U1YangMills):
    ...
    solver_form: str = "normal"  # {normal, split, eo_split}
    ...
    def solve_eop_even_direct(self, U: Array, rhs: Array) -> Array:
        def solve_one(Ub, pb):
            rhs_even = self._project_even(pb)

            def A(x):
                xb = x[None, ...]
                yb = self.apply_eo_schur_even(Ub[None, ...], xb, normalized=True)
                y = yb[0]
                return self._project_even(y) + self._project_odd(x)

            xb, _ = self._solve_linear(A, rhs_even, operator_tag="eop_direct")
            return self._project_even(xb)
    ...
    def solve_eop_even_dagger(self, U: Array, rhs: Array) -> Array:
        def solve_one(Ub, pb):
            ...
            def A(x):
                xb = x[None, ...]
                yb = self.apply_eo_schur_even_dagger(Ub[None, ...], xb, normalized=True)
                y = yb[0]
                return self._project_even(y) + self._project_odd(x)

            xb, _ = self._solve_linear(A, rhs_even, operator_tag="eop_dagger")
            return self._project_even(xb)
    ...
    def solve_eop_even_normal(self, U: Array, rhs: Array) -> Array:
        def solve_one(Ub, pb):
            ...
            def A(x):
                xb = x[None, ...]
                yb = self.apply_eo_schur_even(Ub[None, ...], xb, normalized=True)
                zb = self.apply_eo_schur_even_dagger(Ub[None, ...], yb, normalized=True)
                z = zb[0]
                return self._project_even(z) + self._project_odd(x)

            xb, _ = self._solve_linear(A, rhs_even, operator_tag="eop_normal")
            return self._project_even(xb)
    ...
    def eop_pseudofermion_force_from_solution(self, U: Array, X_e: Array, Y_e: Array) -> Array:
        ...
        k_oe_x = self.apply_K_oe(U, X_e)
        kdag_oe_y = self.apply_Kdag_oe(U, Y_e)
...
def _gauge_transform_fermion(psi: Array, Omega: Array) -> Array:
    return jnp.conj(Omega)[..., None, None] * psi


def _identity_links(th: U1WilsonNf2) -> Array:
    return jnp.ones(th.field_shape(), dtype=th.dtype)
...
def test_gamma_sparse_dense(th: U1WilsonNf2) -> Dict[str, float]:
...
def test_eo_operator(th: U1WilsonNf2, n_iter: int = 10) -> Dict[str, float]:
    U = th.hot_start(scale=0.05)
    psi = th.random_fermion()
    ...
    return {
        ...
        "rel_even_schur_compact_vs_ref": float("nan"),
        "rel_even_norm_schur_vs_ref": rel_se_n,
        "rel_odd_norm_schur_vs_ref": rel_so_n,
        "rel_even_norm_schur_compact_vs_ref": float("nan"),
        "eo_even_sec_per_call": (t1 - t0) / max(1, n_iter),
        "eo_even_compact_sec_per_call": float("nan"),
        "normal_even_sec_per_call": (t3 - t2) / max(1, n_iter),
        "eo_over_normal_even": ((t1 - t0) / max(1, n_iter)) / (((t3 - t2) / max(1, n_iter)) + 1e-16),
        "eo_compact_over_full_even": float("nan"),
    }


def test_hamiltonian_monomials(th: U1WilsonNf2) -> Dict[str, float]:
    U = th.hot_start(scale=0.05)
...
def test_twoflavor_gauge_invariance(
    ...
    for _ in range(max(1, int(n_trials))):
        ...
        Om = random_site_gauge(th, scale=omega_scale)
...
def test_dirac_conventions(th: U1WilsonNf2) -> Dict[str, float]:
    U = th.hot_start(scale=0.05)
    psi = th.random_fermion()
...
def main():
    ...
    ap.add_argument("--gauge-exp-method", type=str, default="u1", choices=["u1", "auto"])
    ...
    ap.add_argument("--pf-fd-iters", type=int, default=2)
    ...
    if "hamiltonian" in tests:
        ...
        print(f"  mean force norm: {h['mean_force_norm']:.6e}")
