...
No extra $1/2$ (or volume/parity) normalization factors are included in
$S_{\mathrm{pf}}$; this is the implemented convention in code.
...
The EO-preconditioned normalization is identical in form:
\begin{equation}
S_{\mathrm{pf}}^{\mathrm{eo}} = \phi_e^\dagger (\hat S_e^\dagger \hat S_e)^{-1}\phi_e
= \eta_e^\dagger \eta_e,
\end{equation}
again with no additional prefactors.
...
\begin{itemize}
  ...
  \item \texttt{ou}: OU update in pseudofermion field space.
\end{itemize}
For unpreconditioned monomials:
\begin{equation}
\phi \leftarrow c_1 \phi + c_2 D^\dagger(U)\zeta,
\qquad
c_1=e^{-\gamma\tau},\quad
c_2=\sqrt{1-c_1^2},
\end{equation}
with fresh Gaussian $\zeta$.
For EO-preconditioned monomials:
\begin{equation}
\phi_e \leftarrow c_1 \phi_e + c_2 \hat S_e^\dagger(U)\zeta_e.
\end{equation}
In OU mode, the implementation keeps $\phi$ (or $\phi_e$) as the dynamical field and
does not transport a stored $\eta$ across changing gauge fields.
...
\begin{itemize}
  ...
  \item Gauge covariance of $\phi$ under the implemented refresh map,
