#!/usr/bin/env python3
"""Component timing benchmark for a single U(1) HMC trajectory.

This script is designed to identify dominant runtime components on realistic
lattices. It times one trajectory in synchronized mode so each component
...
def _compare_reference_kernels(theory, q, p, dt: float, n_iter: int):
    ...
    has_action_opt = hasattr(theory, "action_optimized_unjitted") and getattr(theory, "layout", "") == "BMXYIJ" and int(getattr(theory, "Nd", -1)) in (2, 4)
    if hasattr(theory, "action_reference") and has_action_opt:
        if bool(getattr(theory, "_use_optimized_action", False)) and bool(getattr(theory, "_action_is_jitted", False)):
    ...
    has_force_opt = hasattr(theory, "force_optimized_unjitted") and getattr(theory, "layout", "") == "BMXYIJ" and int(getattr(theory, "Nd", -1)) in (2, 4)
    if hasattr(theory, "force_reference") and has_force_opt:
        if bool(getattr(theory, "_use_optimized_force", False)) and bool(getattr(theory, "_force_is_jitted", False)):
...
def main():
    ap = argparse.ArgumentParser(description="Benchmark one U1 HMC trajectory by component timing.")
    ...
    ap.add_argument("--jit-evolve-q", action=argparse.BooleanOptionalAction, default=True, help="jit U1 evolve_q kernel")
    ...
    ap.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="run one unmeasured trajectory first so first-call JIT compile time is excluded",
    )
