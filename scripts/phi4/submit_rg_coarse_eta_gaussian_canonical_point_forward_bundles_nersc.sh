...
usage() {
  cat <<'EOF'
Usage:
  scripts/phi4/submit_rg_coarse_eta_gaussian_canonical_point_forward_bundles_nersc.sh [options]

Submit the canonical-point uniform-model forward-loss training campaign on
NERSC as regular-qos bundles, grouped by common volume.
...
Optional:
  ...
  --bundle-root PATH    Bundle root. Default:
                        /global/cfs/cdirs/hadron/jaxQFT/runs/phi4/bundles/canonical-point-scan-forward/uniform
...
bundle_root="/global/cfs/cdirs/hadron/jaxQFT/runs/phi4/bundles/canonical-point-scan-forward/uniform"
  ...
  while IFS=$'\t' read -r label mass note; do
    ...
    for width in "${widths[@]}"; do
      for seed in "${seeds[@]}"; do
        ...
        task_name="phi4-${label}-w${width}-L${L}-s${seed}"
...
if [[ ${print_only} -eq 0 ]]; then
  echo "Submitted ${#submitted[@]} bundled forward canonical-point jobs for widths ${widths_csv}."
fi
