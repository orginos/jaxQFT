...
def main():
    ...
    arch = {
        ...
        "eta_gaussian": str(cfg["arch"]["eta_gaussian"]) if args.resume else args.eta_gaussian,
        "gaussian_radius": (
            int(cfg["arch"]["gaussian_radius"])
            if args.resume
            else int(args.radius if args.gaussian_radius < 0 else args.gaussian_radius)
        ),
        "gaussian_width": (
            int(cfg["arch"]["gaussian_width"])
            if args.resume
            else int(args.width if args.gaussian_width <= 0 else args.gaussian_width)
        ),
        "terminal_prior": str(cfg["arch"]["terminal_prior"]) if args.resume else args.terminal_prior,
        "terminal_n_layers": (
    ...
    pbar = tqdm(range(start_epoch, int(args.epochs)), desc="Training(RG Coarse Eta Gaussian Flow)")
