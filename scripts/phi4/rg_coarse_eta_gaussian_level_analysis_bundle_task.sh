...
WRAPPER="${WRAPPER:-${REPO_ROOT}/scripts/phi4/run_rg_coarse_eta_gaussian_level_analysis.sh}"
...
if [[ ! -f "${checkpoint}" ]]; then
  printf '2\n' > "${status_file}"
  echo "Checkpoint not found for task ${task_name}: ${checkpoint}" >&2
  exit 2
fi
...
{
  printf "%s\t%s\t%s\t%s" "${task_name}" "${checkpoint}" "${run_dir}" "${seed}"
  if [[ ${#fields[@]} -gt 4 ]]; then
...
set +e
{
  echo "Launching analysis task ${task_name}"
