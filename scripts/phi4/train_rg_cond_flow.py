#!/usr/bin/env python3
"""Train the RG-first conditional transformer flow for phi^4."""
...
def loss_fn(cfg, weights, key, batch_size, theory):
    z = rg_cond_flow_prior_sample(key, cfg, batch_size)
    x = rg_cond_flow_g(cfg, z, weights)
    return jnp.mean(rg_cond_flow_log_prob(cfg, x, weights) + theory.action(x))
...
def validate(cfg, weights, theory, key, batch_size, super_batch, tag):
    ...
    for _ in range(super_batch):
        ...
        z = rg_cond_flow_prior_sample(k, cfg, batch_size)
        x = rg_cond_flow_g(cfg, z, weights)
        ds = rg_cond_flow_log_prob(cfg, x, weights) + theory.action(x)
        ds = np.asarray(ds)
    ...
    plt.savefig(f"phi4_rg_cond_rw_{tag}.pdf")
    plt.close()

    plt.hist(centered, bins=max(20, int(w.shape[0] / 10)))
    plt.title("DeltaS distribution")
    plt.savefig(f"phi4_rg_cond_ds_{tag}.pdf")
    plt.close()
    return key
...
def main():
    ...
    ap.add_argument("--save", type=str, default="rg_cond_flow_phi4_ckpt.pkl")
    ...
    args = ap.parse_args()
    default_save = "rg_cond_flow_phi4_ckpt.pkl"
    ...
    if args.resume:
        ...
        arch = ckpt["arch"]
        resume_parity = str(arch.get("parity", "none"))
        ...
        model = init_rg_cond_flow(
            ...
            terminal_n_layers=arch.get("terminal_n_layers", 3),
            terminal_width=arch.get("terminal_width", arch["width"]),
            output_init_scale=arch.get("output_init_scale", 1e-2),
            # Legacy checkpoints predate parity metadata and were unsymmetrized.
            parity=resume_parity,
        )
        cfg = model["cfg"]
    ...
    arch = {
        ...
        "n_couplings": args.n_couplings if not args.resume else int(ckpt["arch"]["n_couplings"]),
        ...
        "terminal_n_layers": (
            ...
            if not args.resume
            else int(ckpt["arch"].get("terminal_n_layers", 3))
        ),
        "terminal_width": (
        ...
        "parity": args.parity if not args.resume else resume_parity,
    ...
    print(
        ...
        f" width={arch['width']} n_couplings={arch['n_couplings']}"
    ...
    pbar = tqdm(range(start_epoch, int(args.epochs)), desc="Training(RG Conditional Flow)")
