#!/usr/bin/env python3
"""Train the RG coarse-lattice fluctuation flow for phi^4."""
...
def loss_fn(cfg, weights, key, batch_size, theory):
    z = rg_coarse_eta_flow_prior_sample(key, cfg, batch_size)
    x = rg_coarse_eta_flow_g(cfg, z, weights)
    return jnp.mean(rg_coarse_eta_flow_log_prob(cfg, x, weights) + theory.action(x))
...
def validate(cfg, weights, theory, key, batch_size, super_batch, tag):
    ...
    for _ in range(super_batch):
        ...
        z = rg_coarse_eta_flow_prior_sample(k, cfg, batch_size)
        x = rg_coarse_eta_flow_g(cfg, z, weights)
        ds = rg_coarse_eta_flow_log_prob(cfg, x, weights) + theory.action(x)
        ds = np.asarray(ds)
    ...
    plt.savefig(f"phi4_rg_coarse_eta_rw_{tag}.pdf")
    plt.close()

    plt.hist(centered, bins=max(20, int(w.shape[0] / 10)))
    plt.title("DeltaS distribution")
    plt.savefig(f"phi4_rg_coarse_eta_ds_{tag}.pdf")
    plt.close()
    return key
...
def main():
    ...
    ap.add_argument("--save", type=str, default="rg_coarse_eta_flow_phi4_ckpt.pkl")
    ...
    args = ap.parse_args()
    default_save = "rg_coarse_eta_flow_phi4_ckpt.pkl"
    ...
    arch = {
        ...
        "width": int(cfg["arch"]["width"]) if args.resume else int(args.width),
        "n_cycles": int(cfg["arch"]["n_cycles"]) if args.resume else int(args.n_cycles),
        "radius": int(cfg["arch"]["radius"]) if args.resume else int(args.radius),
        "terminal_n_layers": (
            int(cfg["arch"]["terminal_n_layers"]) if args.resume else int(args.terminal_n_layers)
        ),
        "terminal_width": (
            int(cfg["arch"]["terminal_width"])
            if args.resume
            else int(args.width if args.terminal_width <= 0 else args.terminal_width)
        ),
        "output_init_scale": float(cfg["arch"]["output_init_scale"]) if args.resume else float(args.output_init_scale),
        "parity": str(cfg["arch"]["parity"]) if args.resume else args.parity,
        "rg_type": str(cfg["arch"]["rg_type"]) if args.resume else args.rg_type,
        "log_scale_clip": float(cfg["log_scale_clip"]) if args.resume else float(args.log_scale_clip),
        "offdiag_clip": float(cfg["offdiag_clip"]) if args.resume else float(args.offdiag_clip),
    }
    ...
    pbar = tqdm(range(start_epoch, int(args.epochs)), desc="Training(RG Coarse Eta Flow)")
