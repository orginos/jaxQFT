#!/usr/bin/env python3
"""Train the RG checkerboard conditional flow for phi^4."""
...
def loss_fn(cfg, weights, key, batch_size, theory):
    z = rg_checkerboard_flow_prior_sample(key, cfg, batch_size)
    x = rg_checkerboard_flow_g(cfg, z, weights)
    return jnp.mean(rg_checkerboard_flow_log_prob(cfg, x, weights) + theory.action(x))
...
def validate(cfg, weights, theory, key, batch_size, super_batch, tag):
    ...
    for _ in range(super_batch):
        ...
        z = rg_checkerboard_flow_prior_sample(k, cfg, batch_size)
        x = rg_checkerboard_flow_g(cfg, z, weights)
        ds = rg_checkerboard_flow_log_prob(cfg, x, weights) + theory.action(x)
        ds = np.asarray(ds)
    ...
    plt.savefig(f"phi4_rg_checkerboard_rw_{tag}.pdf")
    plt.close()

    plt.hist(centered, bins=max(20, int(w.shape[0] / 10)))
    plt.title("DeltaS distribution")
    plt.savefig(f"phi4_rg_checkerboard_ds_{tag}.pdf")
    plt.close()
    return key
...
def main():
    ...
    ap.add_argument("--save", type=str, default="rg_checkerboard_flow_phi4_ckpt.pkl")
    ...
    args = ap.parse_args()
    default_save = "rg_checkerboard_flow_phi4_ckpt.pkl"
    ...
    pbar = tqdm(range(start_epoch, int(args.epochs)), desc="Training(RG Checkerboard Flow)")
