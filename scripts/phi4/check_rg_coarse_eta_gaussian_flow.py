#!/usr/bin/env python3
"""Validation and timing harness for the RG coarse-eta Gaussian flow."""
...
def run_invertibility_test(model, shape: tuple[int, int], batch_size: int, tol: float) -> dict:
    ...
    z = rg_coarse_eta_gaussian_flow_prior_sample(key, model, batch_size)
    x = rg_coarse_eta_gaussian_flow_g(model, z)
    zz, ldj = rg_coarse_eta_gaussian_flow_f(model, x)
...
def run_jacobian_test(model, shape: tuple[int, int], tol: float, perturb_scale: float) -> dict:
    ...
    def flat_inverse(flat_x):
        ...
        z, _ = rg_coarse_eta_gaussian_flow_f(cfg, xx, weights)
        return z.reshape((-1,))

    z, ldj = rg_coarse_eta_gaussian_flow_f(cfg, x, weights)
...
def run_timing_test(model, theory, batch_size: int, warmup: int, n_iter: int) -> dict:
    ...
    eta_gauss_g_jit = jax.jit(lambda ee, cc: _eta_gaussian_g(cfg, weights, ee, cc, 0)) if weights["levels"] else None
    eta_gauss_f_jit = jax.jit(lambda ee, cc: _eta_gaussian_f(cfg, weights, ee, cc, 0)) if weights["levels"] else None
    ...
    full_g_jit = jax.jit(lambda zz: rg_coarse_eta_gaussian_flow_g(cfg, zz, weights))
    full_f_jit = jax.jit(lambda xx: rg_coarse_eta_gaussian_flow_f(cfg, xx, weights))
    logp_jit = jax.jit(lambda xx: rg_coarse_eta_gaussian_flow_log_prob(cfg, xx, weights))

    def loss_fn(w):
        key = jax.random.PRNGKey(7)
        zz = rg_coarse_eta_gaussian_flow_prior_sample(key, cfg, batch_size)
        xx = rg_coarse_eta_gaussian_flow_g(cfg, zz, w)
        return jnp.mean(rg_coarse_eta_gaussian_flow_log_prob(cfg, xx, w) + theory.action(xx))
    ...
    if weights["levels"]:
        items.extend(
            [
                ("eta_gaussian_g", eta_gauss_g_jit, (eta, coarse)),
                ("eta_gaussian_f", eta_gauss_f_jit, (eta, coarse)),
...
def main():
    ap = argparse.ArgumentParser(description="Check the RG coarse-eta Gaussian flow.")
    ...
    print(
        ...
        f" eta_gaussian={args.eta_gaussian}"
        f" gaussian_radius={args.radius if args.gaussian_radius < 0 else args.gaussian_radius}"
        f" gaussian_width={args.width if args.gaussian_width <= 0 else args.gaussian_width}"
        f" terminal_prior={args.terminal_prior}"
    ...
    jac_model = init_rg_coarse_eta_gaussian_flow(
        key,
        size=jac_shape,
        width=args.width,
        n_cycles=args.n_cycles,
