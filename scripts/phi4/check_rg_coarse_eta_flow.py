#!/usr/bin/env python3
"""Validation and timing harness for the RG coarse-lattice fluctuation flow."""
...
from jaxqft.models.phi4_rg_coarse_eta_flow import (
    _color_sweep_f,
    _color_sweep_g,
    _conditional_params,
    _eta_context,
...
def run_invertibility_test(model, shape: tuple[int, int], batch_size: int, tol: float) -> dict:
    ...
    z = rg_coarse_eta_flow_prior_sample(key, model, batch_size)
    x = rg_coarse_eta_flow_g(model, z)
    zz, ldj = rg_coarse_eta_flow_f(model, x)
...
def run_jacobian_test(model, shape: tuple[int, int], tol: float, perturb_scale: float) -> dict:
    ...
    def flat_inverse(flat_x):
        ...
        z, _ = rg_coarse_eta_flow_f(cfg, xx, weights)
        return z.reshape((-1,))

    z, ldj = rg_coarse_eta_flow_f(cfg, x, weights)
...
def run_timing_test(model, theory, batch_size: int, warmup: int, n_iter: int) -> dict:
    ...
    cycle_weights = weights["levels"][0][0] if weights["levels"] else None
    if weights["levels"]:
        red_mask, _ = _site_masks(cfg, 0)
        context = _eta_context(eta * (1.0 - red_mask), coarse, cfg["rg_mode"])
    else:
        context = None
    ...
    cond_jit = jax.jit(lambda cc: _conditional_params(cfg, cycle_weights["red"], cc)) if weights["levels"] else None
    red_g_jit = (
        jax.jit(lambda ee, cc: _color_sweep_g(cfg, cycle_weights, ee, cc, 0, "red")) if weights["levels"] else None
    )
    black_f_jit = (
        jax.jit(lambda ee, cc: _color_sweep_f(cfg, cycle_weights, ee, cc, 0, "black")) if weights["levels"] else None
    )
    eta_g_jit = jax.jit(lambda ee, cc: _eta_flow_g(cfg, weights, ee, cc, 0)) if weights["levels"] else None
    eta_f_jit = jax.jit(lambda ee, cc: _eta_flow_f(cfg, weights, ee, cc, 0)) if weights["levels"] else None
    ...
    full_g_jit = jax.jit(lambda zz: rg_coarse_eta_flow_g(cfg, zz, weights))
    full_f_jit = jax.jit(lambda xx: rg_coarse_eta_flow_f(cfg, xx, weights))
    logp_jit = jax.jit(lambda xx: rg_coarse_eta_flow_log_prob(cfg, xx, weights))

    def loss_fn(w):
        key = jax.random.PRNGKey(7)
        zz = rg_coarse_eta_flow_prior_sample(key, cfg, batch_size)
        xx = rg_coarse_eta_flow_g(cfg, zz, w)
        return jnp.mean(rg_coarse_eta_flow_log_prob(cfg, xx, w) + theory.action(xx))
    ...
    if weights["levels"]:
        items.extend(
            [
                ("conditional_params", cond_jit, (context,)),
                ("color_sweep_red_g", red_g_jit, (eta, coarse)),
                ("color_sweep_black_f", black_f_jit, (eta, coarse)),
                ("eta_flow_g", eta_g_jit, (eta, coarse)),
                ("eta_flow_f", eta_f_jit, (eta, coarse)),
            ]
        )
    items.extend(
        [
...
def main():
    ap = argparse.ArgumentParser(description="Check the RG coarse-lattice fluctuation flow.")
    ...
    print(
        ...
        f" offdiag_clip={args.offdiag_clip}"
    )

    key = jax.random.PRNGKey(42)
    model = init_rg_coarse_eta_flow(
        key,
        size=shape,
        width=args.width,
    ...
    jac_model = init_rg_coarse_eta_flow(
        key,
        size=jac_shape,
        width=args.width,
        n_cycles=args.n_cycles,
