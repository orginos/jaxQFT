...
usage() {
  cat <<'EOF'
Usage:
  scripts/phi4/run_rg_coarse_eta_gaussian_debug.sh [wrapper options] [-- trainer args...]

Wrapper options:
  --mode interactive|debug   Select the default config card if --config is omitted.
  ...
  --no-activate              Do not load the NERSC python module or source ~/venv(s)/jax.
...
Examples:
  scripts/phi4/run_rg_coarse_eta_gaussian_debug.sh --mode interactive -- --save debug_checkpoint.pkl
  scripts/phi4/run_rg_coarse_eta_gaussian_debug.sh --config configs/phi4/paper-2/canonical-scaling/L64_uniform.toml -- --max-epochs 10
EOF
}
...
cache_dir="${autotune_root}/phi4_rg_coarse_eta_gaussian_debug"
mkdir -p "${MPLCONFIGDIR}" "${cache_dir}"

append_xla_flag "--xla_gpu_per_fusion_autotune_cache_dir=${cache_dir}"
if [[ ${no_triton} -eq 1 ]]; then
  append_xla_flag "--xla_gpu_enable_triton_gemm=false"
fi

cmd=(
  "${python_exe}"
  "${repo_root}/scripts/phi4/train_rg_coarse_eta_gaussian_flow_debug.py"
  "--config" "${config}"
)
