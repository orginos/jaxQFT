#!/usr/bin/env python3
"""Validation and timing harness for the RG checkerboard in-block phi^4 flow."""
...
def run_invertibility_test(model, shape: tuple[int, int], batch_size: int, tol: float) -> dict:
    ...
    z = rg_checkerboard_inblock_flow_prior_sample(key, model, batch_size)
    x = rg_checkerboard_inblock_flow_g(model, z)
    zz, ldj = rg_checkerboard_inblock_flow_f(model, x)
...
def run_jacobian_test(model, shape: tuple[int, int], tol: float, perturb_scale: float) -> dict:
    ...
    def flat_inverse(flat_x):
        ...
        z, _ = rg_checkerboard_inblock_flow_f(cfg, xx, weights)
        return z.reshape((-1,))

    z, ldj = rg_checkerboard_inblock_flow_f(cfg, x, weights)
...
def run_timing_test(model, theory, batch_size: int, warmup: int, n_iter: int) -> dict:
    ...
    if weights["levels"]:
        red_mask = cfg["site_red"][0]
        black_mask = cfg["site_black"][0]
        inblock_mask = cfg["inblock_masks"][0].reshape((1, 1, 1, 3))
        context = _masked_eta_context(eta * (black_mask + red_mask * inblock_mask), coarse, cfg["rg_mode"])
    else:
        context = None
    ...
    trf_jit = jax.jit(lambda cc: _conditional_st(cfg, pass_weights["red"][0], cc)) if weights["levels"] else None
    color_red_g_jit = (
        jax.jit(lambda ee, cc: _color_pass_g(cfg, pass_weights, ee, cc, 0, "red")) if weights["levels"] else None
    )
    color_black_f_jit = (
        jax.jit(lambda ee, cc: _color_pass_f(cfg, pass_weights, ee, cc, 0, "black")) if weights["levels"] else None
    )
    ...
    full_g_jit = jax.jit(lambda zz: rg_checkerboard_inblock_flow_g(cfg, zz, weights))
    full_f_jit = jax.jit(lambda xx: rg_checkerboard_inblock_flow_f(cfg, xx, weights))
    logp_jit = jax.jit(lambda xx: rg_checkerboard_inblock_flow_log_prob(cfg, xx, weights))

    def loss_fn(w):
        key = jax.random.PRNGKey(7)
        zz = rg_checkerboard_inblock_flow_prior_sample(key, cfg, batch_size)
        xx = rg_checkerboard_inblock_flow_g(cfg, zz, w)
        return jnp.mean(rg_checkerboard_inblock_flow_log_prob(cfg, xx, w) + theory.action(xx))
    ...
    if weights["levels"]:
        items.extend(
            [
                ...
                ("color_pass_red_g", color_red_g_jit, (eta, coarse)),
                ("color_pass_black_f", color_black_f_jit, (eta, coarse)),
...
def main():
    ap = argparse.ArgumentParser(description="Check the RG checkerboard in-block phi^4 flow.")
    ...
    print(
        ...
        f" n_inner_couplings={args.n_inner_couplings}"
    ...
    jac_model = init_rg_checkerboard_inblock_flow(
        key,
        size=jac_shape,
        width=args.width,
        n_cycles=args.n_cycles,
